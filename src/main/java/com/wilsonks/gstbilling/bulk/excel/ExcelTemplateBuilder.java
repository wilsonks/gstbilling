...
public class ExcelTemplateBuilder {
    ...
    public byte[] buildTemplate(
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            log.error(
                    "Failed generating template",
                    ex);

            throw new IllegalStateException(
                    "Failed generating Excel template",
                    ex);
        }
    }
    ...
    private void createInstructionSheet(
        ...
        for (ExcelColumn column : columns) {
            ...
            row.createCell(3)
                    .setCellValue(
                            safe(column.getDescription()));

            row.createCell(4)
                    .setCellValue(
                            safe(column.getExample()));
        }
